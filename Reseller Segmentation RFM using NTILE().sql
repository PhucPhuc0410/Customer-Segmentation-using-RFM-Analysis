...
-- Recency Calculation (Assume current date is 2013/11/29)
...
SELECT
    ...
    NTILE(5) OVER (ORDER BY COUNT(DISTINCT SalesOrderNumber) ASC) AS Frequency
...
SELECT
    ...
    NTILE(5) OVER (ORDER BY COUNT(DISTINCT SalesAmount) ASC) AS Monetary
